...
const Button = props => {
    ...
    return (
        <div className={`flex justify-center content-center ${props.sectionClass}`}>
            <button 
                type={props.type}
                className={`${props.shadow || "drop-shadow-title"} ${props.width || "w-44"} py-1 ${props.borderRadious || "rounded-lg"} ${props.className} ${props.isDisabled ? ("bg-pgray text-dusty-gray border-pgray cursor-not-allowed") : (props.bgColor || "bg-turbo")}`}
